...
IncidenceIterator::IncidenceIterator(std::vector<Incidence *>::const_iterator i) : i_(i) {}
IncidenceIterator::IncidenceIterator() {}
...
Rotation::Rotation(const Vertex &v, Number n): v_(v), n_(n) {}
Rotation::~Rotation() {
    for(Incidence *p:rr_)
        delete(p);
}
...
std::set<Number> Rotation::neighbours() const {
