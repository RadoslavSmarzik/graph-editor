...
RotationIterator::RotationIterator(std::map<int,Rotation *>::const_iterator i) : i_(i) {}
RotationIterator::RotationIterator() {}
...
Graph::Graph(Factory * f): my_factory(f), edge_count(0), free_number(0) {}
#else
Graph::Graph(): edge_count(0), free_number(0) {}
#endif
Graph::~Graph() {for(auto it=begin();it!=end();it++) delete(&(*it));}
Graph::Graph (Graph &&g) noexcept: edge_count(g.edge_count), free_number(g.free_number),
        rr_(std::move(g.rr_)) {
#ifdef BA_GRAPH_DEBUG
