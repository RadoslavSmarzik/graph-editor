...
Graph random_pseudograph(int order, int size, Factory &f=static_factory) {
#ifdef BA_GRAPH_DEBUG
    assert(order >= 0); assert(size >= 0);
#endif
    Graph G(empty_graph(order));
    for (; size > 0; --size) {
        int u = rand() % order;
        int v = rand() % order;
...
Graph random_multigraph(int order, int size, Factory &f=static_factory) {
#ifdef BA_GRAPH_DEBUG
    assert(order >= 0); assert(size >= 0);
#endif
    Graph G(empty_graph(order));
...
Graph random_simple_graph_unreliable(int order, int maxSize, Factory &f=static_factory) {
#ifdef BA_GRAPH_DEBUG
...
int randomAvailableInt(std::vector<bool> a) {
    int aCount = 0;
...
Graph random_simple_graph_reliable(int order, int size, Factory &f=static_factory) {
#ifdef BA_GRAPH_DEBUG
    assert(order >= 0); assert(size >= 0);
#endif
    Graph G(empty_graph(order));
...
Graph random_regular_multigraph(int order, int degree, Factory &f=static_factory) {
#ifdef BA_GRAPH_DEBUG
...
Graph random_bipartite_regular_multigraph(int order, int degree, Factory &f=static_factory) {
#ifdef BA_GRAPH_DEBUG
