...
int distance(const Graph &G, Number v1, Number v2, int cutoff=INT_MAX)
{
    ...
    while (!finished && !q.empty()) {
        ...
        for (auto &i : r) {
            ...
            if (dist[neighbour.n()] == -1) {
                q.push(&neighbour);
                dist.set(neighbour.n(), dist[r.n()] + 1);
                if (r.n() == v2) {
                    finished = true;
                    break;
                }
...
// distances from a set of vertices to the remaining vertices
GraphNumberLabeling<int> distances(const Graph &G, std::set<Number> startingPoints)
{
#ifdef BA_GRAPH_DEBUG
...
GraphNumberLabeling<int> distances(const Graph &G, Number w)
{
#ifdef BA_GRAPH_DEBUG
    assert(G.contains(w));
#endif
...
int distance(const Graph &G, Location l1, Location l2, int cutoff = INT_MAX)
{
#ifdef BA_GRAPH_DEBUG
...
int distance(const Graph &G, Edge e1, Edge e2, int cutoff = INT_MAX)
{
#ifdef BA_GRAPH_DEBUG
...
std::vector<std::vector<Number>> components(const Graph &G)
{
    ...
    while (notProcessed.size() > 0) {
        Number v = *notProcessed.begin();
        auto d = distances(G, v);
        std::vector<Number> component;
        for (auto it = notProcessed.begin(); it != notProcessed.end(); ) {
            if (d[*it] != -1) {
                component.push_back(*it);
                it = notProcessed.erase(it);
            } else {
                it++;
            }
        }
