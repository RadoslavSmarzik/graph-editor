...
bool has_cut_vertex(const Graph &G)
{
    Factory f;
...
std::vector<Number> cut_vertices(const Graph &G)
{
...
bool has_cut_edge(const Graph &G)
{
    Factory f;
    auto H = copy_other_factory(G, f);
...
std::vector<Location> cut_edges(const Graph &G)
{
