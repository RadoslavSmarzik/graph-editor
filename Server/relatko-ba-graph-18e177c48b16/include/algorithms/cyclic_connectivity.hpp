...
vector<int> getCut(const Graph &G,const Vertex &V,  int forb);
vector<int> getCoAtoms(const Graph &G,const vector<int> &atom);
...
vector<int> getCoAtoms(const Graph &G,
...
Result ford_Fulk_T(Graph &G, vector<FTreeNode> &a, vector<FTreeNode> &b) {
	...
	set<int> targets;
	for (vector<FTreeNode>::size_type i = 0; i < b.size(); i++) {
		targets.insert(b.at(i).number);
	}
	for (vector<FTreeNode>::size_type i = 0; i < a.size(); i++) {
...
std::tuple<Graph *, ba_graph::Graph, Mapping<Number>> ba_graph_to_mrozek(const ba_graph::Graph &g)
{
...
std::pair<int, std::set<std::set<Edge>>> main_mrozek(const ba_graph::Graph &g) {
...
bool main_mrozek_is_cc(const ba_graph::Graph &g, int cc) {
	auto [G, Gba, newToOriginal] = ba_graph_to_mrozek(g);
...
int cyclic_connectivity(const Graph &G)
{
...
bool is_cyclically_connected(const Graph &G, int k)
{
#ifdef BA_GRAPH_DEBUG
	assert(k >= 1);
	assert(max_deg(G) == 3);
...
std::pair<int, std::set<std::set<Edge>>> cyclic_connectivity_with_cuts(const Graph &G)
{
#ifdef BA_GRAPH_DEBUG
	assert(max_deg(G) == 3);
	assert(min_deg(G) == 3);
...
bool contains_cc_obstacle(const Graph &G, const Multipole &mG, int requiredCC)
{
#ifdef BA_GRAPH_DEBUG
	...
	int g = girth(G);
	if (g != -1) {
		if (g < requiredCC) return true; // small girth means small cut
