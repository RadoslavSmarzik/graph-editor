...
bool minimum_uncovered_vertices_callback(std::vector<PFPath> paths, struct MinimumUncoveredVerticesCallbackData *cbData) {
...
int minimum_uncovered_vertices(const Graph &G, std::vector<std::pair<Number, Number>> endpoints,
        std::set<Number> ignored = {})
{
#ifdef BA_GRAPH_DEBUG
...
int is_hamiltonian_basic(const Graph &G, bool ignoreIsolated = false)
{
...
bool is_hypohamiltonian_basic(const Graph &G, bool checkHamiltonicity = true)
{
...
std::ostream &operator <<(std::ostream &out, Circuit c)
{
    out << c.vertices();
    return out;
}

std::set<Path> all_paths(const Graph &G, int length)
{
#ifdef BA_GRAPH_DEBUG
    assert(length >= 0);
#endif
...
std::set<Path> all_paths(const Graph &G, int lengthFrom, int lengthTo)
{
#ifdef BA_GRAPH_DEBUG
...
std::set<Path> all_paths(const Graph &G)
{
    return all_paths(G, 0, G.order());
}

std::set<Circuit> all_circuits(const Graph &G, int length)
{
#ifdef BA_GRAPH_DEBUG
    assert(3 <= length);
#endif
...
std::set<Circuit> all_circuits(const Graph &G, int lengthFrom, int lengthTo)
{
#ifdef BA_GRAPH_DEBUG
    assert(0 <= lengthFrom && lengthFrom <= lengthTo);
#endif
...
std::set<Circuit> all_circuits(const Graph &G)
{
