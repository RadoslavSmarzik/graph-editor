...
int vector_hash(std::vector<int> v, int seed=-1)
{
...
IsomorphismVertexHash isomorphism_vertex_hash(const Graph &G, Number v)
{
...
int isomorphism_vertex_hash_int(const Graph &G, Number v)
{
	auto h = isomorphism_vertex_hash(G, v);
	return vector_hash(h.second, h.first * h.second.size());
}

int isomorphism_graph_hash(const Graph &G)
{
