...
Graph empty_graph(int cnt, Factory &f = static_factory)
{
...
Graph complete_graph(int cnt, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
    assert(cnt >= 0);
#endif
...
Graph path(int length, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
    assert(length >= 0);
#endif
...
Graph circuit(int length, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
    assert(length >= 0);
#endif
