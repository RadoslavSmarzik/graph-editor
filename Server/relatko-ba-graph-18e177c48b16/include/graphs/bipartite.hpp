...
Graph complete_bipartite_graph(int a, int b, Factory &f = static_factory)
{
...
Graph square_grid(int m, int n, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
    assert((m >= 0) && (n >= 0));
#endif
...
Graph wrapped_square_grid(int m, int n, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
    assert((m >= 0) && (n >= 0));
#endif
