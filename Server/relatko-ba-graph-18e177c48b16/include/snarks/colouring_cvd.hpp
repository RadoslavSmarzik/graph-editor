...
int color_cvd_param(char *mat, int size, int iterlimit, int replimit)
{
    int ret;
...
int color_cvd_adjlist(int *mat, int *colormat, int size)
{
    int ret;
    struct graph g = graph_init(size);
    for (int u = 0; u < size; u++) {
...
bool is3EdgeColourable_cvd(const Graph &G, int iterationLimit=-1, int repetitionLimit=-1)
{
    Factory f;
