...
std::string print_nice_only_primary(const Graph &G)
{
	std::stringstream ss;
	std::vector<Number> vertices;
	vertices.reserve(G.order());
...
std::string print_nice_vertices(const Graph &G)
{
	std::stringstream ss;
	ss << G.order() << " vertices: ";
...
std::string neato_representation(const Graph &G, std::vector<Number> &redVertices)
{
	std::stringstream ss;
...
void visualise_neato(const std::string outputFileName, const Graph &G, std::vector<Number> redVertices = {})
{
