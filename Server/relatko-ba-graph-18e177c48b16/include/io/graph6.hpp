...
int decode_graph6_order(std::string::iterator s, std::string::iterator sEnd)
{
...
std::string encode_graph6_order(int n)
{
	std::stringstream s;
	if (n < 64) {
...
Graph read_graph6(std::string::iterator s, std::string::iterator sEnd, Factory &f = static_factory)
{
...
// incremental sparse6 not supported
Graph read_sparse6(std::string::iterator s, std::string::iterator sEnd, Factory &f = static_factory)
{
	int n = internal::decode_graph6_order(s, sEnd);
	s += internal::graph6_order_length(n);
	Graph G(empty_graph(n, f));
...
Graph read_digraph6(std::string::iterator s, std::string::iterator sEnd, Factory &f = static_factory)
{
...
Graph read_graph6_line(std::string line, Factory &f = static_factory)
{
	if (line[0] == '>') {
...
MinimizedGraphs read_graph6_stream(std::istream &in,
 ...
 */
int read_graph6_file_count(const std::string &fileName, int from = 0, int to = INT_MAX)
{
	std::ifstream fin(fileName);
	if (!fin)
		throw std::runtime_error("cannot read file " + fileName);
 ...
 */
MinimizedGraphs read_graph6_file(const std::string &fileName,
		Factory &f = static_factory, int from = 0, int to = INT_MAX, int step = 1)
{
#ifdef BA_GRAPH_DEBUG
	assert((0 <= from) && (from <= to) && (step > 0));
...
void write_graph6_stream(const Graph &G, std::ostream &out, bool includeNewline = true)
{
	const int n = G.order();
	Factory f;
...
std::string write_graph6(const Graph &G, bool includeNewline = true)
{
	std::ostringstream s;
...
void write_graph6_file(const std::string fileName, const Graph &G, bool append = false)
{
	std::ofstream fout;
	if (append)
		fout.open(fileName, std::ios_base::app);
...
std::vector<int> write_sparse6_enc(int k, int x)
{
...
void write_sparse6_stream(const Graph &G, std::ostream &out, bool includeNewline = true)
{
	const int n = G.order();
...
std::string write_sparse6(const Graph &G, bool includeNewline = true)
{
	std::ostringstream s;
...
void write_sparse6_file(const std::string fileName, const Graph &G, bool append = false)
{
	std::ofstream fout;
	if (append)
		fout.open(fileName, std::ios_base::app);
