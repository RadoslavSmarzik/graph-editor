...
std::string getline_ba(std::istream &in) {
    std::string s;
    while (1) {
        std::getline(in, s);
...
Graph read_oldBA_from_stream(std::istream &in, bool numberRemoved, Factory &f=static_factory) {
    if (!numberRemoved)
    ...
    for (int i = 0; i < order; i++) {
        line = std::stringstream(internal::getline_ba(in));
        for (int j = 0; j < 3; j++) { // TODO do we only support cubic graphs?
            int v; line >> v;
            if (i < v)
...
std::vector<Graph> read_oldBA_stream(std::istream &in, Factory &f=static_factory) {
    int count;
    std::stringstream line = std::stringstream(internal::getline_ba(in));
    line >> count;
...
std::vector<Graph> read_oldBA_file(const std::string &fileName, Factory &f=static_factory) {
...
std::string write_oldBA(const Graph &G) {
    Factory f;
