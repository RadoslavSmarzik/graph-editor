...
void join(Graph &G, Multipole &c, int con, const Graph &g1, const Multipole &c1, int con1,
...
void join(Graph &G, Multipole &c, int con, int con1, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
    assert(check(G, f));
    assert(con != con1);
