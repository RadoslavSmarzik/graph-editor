...
Multipole multipole_by_splitting(Graph &G, const std::vector<Number> &verticesToSplit,
...
Multipole multipole_by_splitting(Graph &G, const std::set<Number> &verticesToSplit,
...
Multipole multipole_by_removing(Graph &G, std::vector<Number> verticesToRemove,
		std::vector<Location> edgesToRemove, Number firstTerminal, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
	assert(firstTerminal >= 0);
	for (auto &v : verticesToRemove)
		assert(G.contains(v));
	for (auto &l : edgesToRemove)
		assert(G.contains(l));
#endif
	...
	for (auto v : verticesToRemove) {
		for (auto &ii : G[v].list(IP::primary())) {
			if (verticesToRemoveSet.count(ii->n2()) > 0)
				locationsToDelete.insert(ii->l());
		}
	}
	std::vector<Location> locationsToSplit;
	for (auto l : edgesToRemove) {
		if (locationsToDelete.count(l) == 0)
			locationsToSplit.push_back(l);
	}
	for (Location l : locationsToDelete)
		deleteE(G, l, f);
	Multipole m = multipole_by_splitting(G, verticesToRemove, locationsToSplit, firstTerminal, f);
	m.flatten();
	return m;
}

Multipole multipole_by_removing(Graph &G, const std::vector<Number> &verticesToRemove,
		const std::vector<Edge> &edgesToRemove, Number firstTerminal, Factory &f = static_factory)
{
	std::vector<Location> locationsToRemove;
	for (Edge e : edgesToRemove) {
#ifdef BA_GRAPH_DEBUG
		G.contains(e.v1(), e);
#endif
		locationsToRemove.push_back(G.find(e.v1(), e).second->l());
	}
	return multipole_by_removing(G, verticesToRemove, locationsToRemove, firstTerminal, f);
}

/* ========================== paths and circuits  ====================== */

std::pair<Graph, Multipole> multipole_path(int length, Number firstTerminal, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
	assert(length >= 1);
...
Multipole multipole_path_complement(Graph &G, std::vector<Number> path, Number firstTerminal, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
	assert(firstTerminal >= 0);
	assert(path.size() >= 1);
...
std::pair<Graph, Multipole> multipole_circuit(int length, Number firstTerminal, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
	assert(length >= 3);
...
Multipole multipole_circuit_complement(Graph &G, std::vector<Number> circuit, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
...
std::pair<Graph, Multipole> multipole_full_tree(int depth, Number firstTerminal, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
	assert(depth >= 0);
...
Multipole multipole_full_tree_complement(Graph &G, Number root, int depth, Number firstTerminal, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
...
Multipole multipole_negator(Graph &G, Number u, Number w, Number v,
...
Multipole multipole_23(Graph &G, Location l, Number v, Number firstTerminal, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
	assert(firstTerminal >= 0);
...
Multipole multipole_odd_222(Graph &G, Number v, Number firstTerminal, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
	assert(G.contains(v));
	assert(G[v].degree() == 3);
...
std::pair<Graph, Multipole> multipole_negator_petersen(Number firstTerminal = 10, Factory &f = static_factory)
{
...
std::pair<Graph, Multipole> multipole_23_petersen(int distance, Number firstTerminal = 10, Factory &f = static_factory)
{
	Graph G(create_petersen(f));
	Location l = Location(0, G[0].find(IP::all())->n2());
	Number v;
	for (auto &r : G) {
		auto d = distances(G, r.n());
		if ((d[l.n1()] >= distance) && (d[l.n2()] >= distance))
			v = r.n();
	}
	return std::pair(std::move(G), multipole_23(G, l, v, firstTerminal, f));
}

std::pair<Graph, Multipole> multipole_odd_222_petersen(Factory &f = static_factory)
{
