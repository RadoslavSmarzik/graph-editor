...
bool has_perfect_matching_sat(const SatSolver &solver, const Graph &G)
{
...
bool has_kfactor_sat(const SatSolver &solver, const Graph &G, int k)
{
...
Graph transform_to_2factor_cubic(std::vector<bool> &solution, struct KFactorTransformData *data)
{
...
std::vector<Graph> two_factors_list_allsat_cubic(const AllSatSolver &solver, const Graph &G, Factory &f)
{
#ifdef BA_GRAPH_DEBUG
    assert(max_deg(G) == 3);
    assert(min_deg(G) == 3);
