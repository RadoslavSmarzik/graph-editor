...
int circular_distance(int p, int c1, int c2) {
...
// it gets that colour
CNF cnf_circular_vertex_colouring(const Graph &G, int p, int q, VertexLabeling<int> precolouring)
{
    ...
    int nextVar = 0;
    for (int i = 0; i < G.order(); ++i) { // FIXME assumes G is numbered consecutively from 0, replace by traversing vertices ordered by number
        auto &r = G[i];
        std::vector<int> v(p);
        for (int j = 0; j < p; ++j)
            v[j] = nextVar++;
...
CNF cnf_circular_vertex_colouring(const Graph &G, int p, int q, bool precolour = false)
{
    VertexLabeling<int> precolouring(-1);
    if (!precolour)
        return cnf_circular_vertex_colouring(G, p, q, precolouring);
    precolouring.set(G.find(RP::all())->v(), 0); // TODO can be improved if only half of the colours were tried for a neighbour
    return cnf_circular_vertex_colouring(G, p, q, precolouring);
}

// for each of the edges in precolouring that has colour from {0, 1, ..., k-1}
// it gets that colour
CNF cnf_circular_edge_colouring(const Graph &G, int p, int q, EdgeLabeling<int> precolouring)
{
    Factory f;
...
CNF cnf_circular_edge_colouring(const Graph &G, int p, int q, bool precolour = false)
{
    Factory f;
    auto [H, edgeToVertex] = line_graph_with_map(G, f);
    VertexLabeling<int> precolouring(-1);
...
std::vector<std::pair<int, int>> computeCircularColouringFractions(const Graph &G, int upper_bound)
{
    int ub = upper_bound;
    int lb = ub - 1;
