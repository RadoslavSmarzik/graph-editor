...
// it gets that colour
CNF cnf_vertex_colouring(const Graph &G, int k, VertexLabeling<int> precolouring)
{
...
CNF cnf_vertex_colouring(const Graph &G, int k, bool precolour = false)
{
...
// it gets that colour
CNF cnf_edge_colouring(const Graph &G, int k, EdgeLabeling<int> precolouring)
{
    Factory f;
...
CNF cnf_edge_colouring(const Graph &G, int k, bool precolour = false)
{
    Factory f;
    auto [H, edgeToVertex] = line_graph_with_map(G, f);
    VertexLabeling<int> precolouring(-1);
