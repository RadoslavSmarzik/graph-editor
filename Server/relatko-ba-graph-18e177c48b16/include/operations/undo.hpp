...
std::vector<ClearedEdge> clear_edges(Graph &G, std::vector<Location> edges, Factory &f = static_factory)
{
...
std::vector<ClearedEdge> clear_edges(Graph &G, std::vector<Edge> edges, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
    assert(check(G, f));
#endif
...
std::vector<ClearedEdge> clear_edges(Graph &G, std::set<Edge> edges, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
    assert(check(G, f));
#endif
...
std::vector<ClearedEdge> clear_edges(Graph &G, std::vector<Number> vertices, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
    assert(check(G, f));
...
void restore_edges(Graph &G, std::vector<ClearedEdge> clearedEdges, Factory &f = static_factory)
{
#ifdef BA_GRAPH_DEBUG
    assert(check(G, f));
#endif
