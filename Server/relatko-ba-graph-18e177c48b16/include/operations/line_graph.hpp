...
std::pair<Graph, EdgeLabeling<Number>> line_graph_with_map(const Graph &G, Factory &f = static_factory)
{
    Graph H(createG(f));
    EdgeLabeling<Number> edgeToVertex;
    int num = 0;
    for (auto e : G.list(RP::all(), IP::primary(), IT::e())) {
        auto v = addV(H, num, f).v();
        edgeToVertex.set(e, num);
        num++;
    }
...
Graph line_graph(const Graph &G, Factory &f = static_factory)
{
